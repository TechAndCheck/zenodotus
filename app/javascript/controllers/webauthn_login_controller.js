...
export default class extends Controller {
  ...
  async authenticateWebauthn() {
    ...
    const setupResponse = await get('/users/sign_in/mfa/webauthn.json', {
      contentType: 'application/json',
      responseKind: 'json',
    })
    ...
    const finishWebauthnResponse = await post(
      '/users/sign_in/mfa/webauthn.json',
      {
    ...
    if (finishedBodyJson.authentication_status == 'success') {
      ...
      let redirectPath = '/'
      if ('redirect' in finishedBodyJson) {
      ...
      this.lockTarget.firstChild.classList.add(
        'transition-opacity',
        'duration-500',
        'ease-out',
      )
  ...
  async authenticateRecoveryCode() {
    ...
    const response = await post(
      '/users/sign_in/mfa/webauthn/use_recovery_code.json',
      {
