# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `actionpack-action_caching` gem.
# Please instead update this file by running `bin/tapioca gem actionpack-action_caching`.


# source://actionpack-action_caching//lib/actionpack/action_caching/railtie.rb#3
module ActionPack
  class << self
    # source://actionpack/7.2.2.1/lib/action_pack/gem_version.rb#7
    def gem_version; end

    # source://actionpack/7.2.2.1/lib/action_pack/version.rb#9
    def version; end
  end
end

# source://actionpack-action_caching//lib/actionpack/action_caching/railtie.rb#4
module ActionPack::ActionCaching; end

# source://actionpack-action_caching//lib/actionpack/action_caching/railtie.rb#5
class ActionPack::ActionCaching::Railtie < ::Rails::Railtie
  class << self
    # source://comma/4.7.0/lib/comma/object.rb#7
    def comma_formats; end
  end
end
